...
bool cycle(int winCand, int loseCand);
...
void sort_pairs(void)
{
    ...
    for (int j = 0; j < pair_count; j++)
    {
        ...
        {
            temp_swap = pairs[j];
            if (pairs[k].strength > pairs[j].strength)
            {
                pairs[j] = pairs[k];
                pairs[k] = temp_swap;
            }
...
// helper function to determine if lock will create cycle
bool cycle(int winCand, int loseCand)
{
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[loseCand][i])
        {
            if (i == winCand)
            {
                return true;
            }
            else if (cycle(winCand, i))
            {
                return true;
            }
        }
